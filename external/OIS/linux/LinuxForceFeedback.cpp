...
void LinuxForceFeedback::_upload( struct ff_effect* ffeffect, const Effect* effect)
{
    ...
    if( linEffect == 0 )
    {
        ...
        if ( mEffectList.find(effect->_handle) !=  mEffectList.end())  {mEffectList.erase(effect->_handle);}
        mEffectList.insert(std::pair<int, struct ff_effect *>(effect->_handle, linEffect));
        #else
