...
bool EventUtils::isJoyStick( int deviceID, JoyStickInfo &js )
{
    ...
    for(vector<int>::iterator i = info.buttons.begin(), e = info.buttons.end(); i != e; ++i )
    {
        ...
        if ( js.button_map.find(*i) !=  js.button_map.end())  js.button_map.erase(*i);
        js.button_map.insert(std::pair<int, int>(*i, buttons++));
        #else
    ...
    if( joyButtonFound )
    {
        ...
        {
            #if defined(__GNUC__) && defined(__INTEL_COMPILER)
            if ( js.axis_map.find(*i) !=  js.axis_map.end())  {js.axis_map.erase(*i);}
            js.axis_map.insert(std::pair<int, int>(*i, axes));
            #else
            js.axis_map[*i] = axes;
            #endif
            ...
            if (ioctl(deviceID, EVIOCGABS(*i), &absinfo) == -1)
                OIS_EXCEPT( E_General, "Could not read device absolute axis features");
            #if defined(__GNUC__) && defined(__INTEL_COMPILER)
            if ( js.axis_range.find(axes) !=  js.axis_range.end())  {js.axis_range.erase(axes);}
            js.axis_range.insert(std::pair<int, Range>(axes, Range(absinfo.minimum, absinfo.maximum)));
            #else
...
#if defined(__GNUC__) && defined(__INTEL_COMPILER)
    typeMap.insert(std::pair<int, Effect::EType>(FF_CONSTANT, Effect::Constant));
    typeMap.insert(std::pair<int, Effect::EType>(FF_RAMP, Effect::Ramp));
    typeMap.insert(std::pair<int, Effect::EType>(FF_SPRING, Effect::Spring));
    typeMap.insert(std::pair<int, Effect::EType>(FF_FRICTION, Effect::Friction));
    typeMap.insert(std::pair<int, Effect::EType>(FF_SQUARE, Effect::Square));
    typeMap.insert(std::pair<int, Effect::EType>(FF_TRIANGLE, Effect::Triangle));
    typeMap.insert(std::pair<int, Effect::EType>(FF_SINE, Effect::Sine));
    typeMap.insert(std::pair<int, Effect::EType>(FF_SAW_UP, Effect::SawToothUp));
    typeMap.insert(std::pair<int, Effect::EType>(FF_SAW_DOWN, Effect::SawToothDown));
    typeMap.insert(std::pair<int, Effect::EType>(FF_DAMPER, Effect::Damper));
    typeMap.insert(std::pair<int, Effect::EType>(FF_INERTIA, Effect::Inertia));
    typeMap.insert(std::pair<int, Effect::EType>(FF_CUSTOM, Effect::Custom));
#else
...
#if defined(__GNUC__) && defined(__INTEL_COMPILER)
    forceMap.insert(std::pair<int, Effect::EForce>(FF_CONSTANT, Effect::ConstantForce));
    forceMap.insert(std::pair<int, Effect::EForce>(FF_RAMP, Effect::RampForce));
    forceMap.insert(std::pair<int, Effect::EForce>(FF_SPRING, Effect::ConditionalForce));
    forceMap.insert(std::pair<int, Effect::EForce>(FF_FRICTION, Effect::ConditionalForce));
    forceMap.insert(std::pair<int, Effect::EForce>(FF_SQUARE, Effect::PeriodicForce));
    forceMap.insert(std::pair<int, Effect::EForce>(FF_TRIANGLE, Effect::PeriodicForce));
    forceMap.insert(std::pair<int, Effect::EForce>(FF_SINE, Effect::PeriodicForce));
    forceMap.insert(std::pair<int, Effect::EForce>(FF_SAW_UP, Effect::PeriodicForce));
    forceMap.insert(std::pair<int, Effect::EForce>(FF_SAW_DOWN, Effect::PeriodicForce));
    forceMap.insert(std::pair<int, Effect::EForce>(FF_DAMPER, Effect::ConditionalForce));
    forceMap.insert(std::pair<int, Effect::EForce>(FF_INERTIA, Effect::ConditionalForce));
    forceMap.insert(std::pair<int, Effect::EForce>(FF_CUSTOM, Effect::CustomForce));
#else
