...
namespace zeug
{
  namespace opengl
  {
    namespace detail
    {
        ...
        static void fast_idct(std::uint8_t *out, std::int32_t out_stride,  std::int16_t data[64], std::uint16_t *dequantize)
        {
            std::int16_t src = ((*data) * (*dequantize);
            ippiDCT8x8Inv_16s8u_C1R(&src, out, out_stride);
        }
         static void fast_ycbcr_to_rgb(std::uint8_t *output, std::uint8_t const *y, std::uint8_t const *cb, std::uint8_t const *cr, std::int32_t count, std::int32_t step)
        {
            std::uint8_t src[] =
            {
                *y, *cb, *cr
            ...
            IppiSize size = {count};
            ippiYCbCr422ToRGB_8u_C2C4R(src, step, output, step, size, 255);
        }     
    }
    ...
    {
        stbi_install_idct(detail::fast_idct);
        stbi_install_YCbCr_to_RGB(detail::fast_ycbcr_to_rgb);
