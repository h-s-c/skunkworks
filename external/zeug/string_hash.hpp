...
{ 
  public:
    ...
    explicit constexpr string_hash(const char* str) : hash(hashString(str)) {}
    explicit constexpr string_hash(const std::int64_t hash) : hash(hash) {}
    ...
    explicit constexpr
    #endif
    string_hash(void* hash) : hash(*(std::int64_t*)(hash)){}
