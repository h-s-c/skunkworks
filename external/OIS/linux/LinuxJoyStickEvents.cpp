    ...
    while(true)
    {
        ...
        for(int i = 0; i < ret; ++i)
        {
            ...
            case EV_KEY:  //Button
            {
                ...
                int button = mButtonMap.find(js[i].code)->second;
                #else
            ...
            case EV_ABS:  //Absolute Axis
            {
                ...
                {
                    #if defined(__GNUC__) && defined(__INTEL_COMPILER)                
                    int axis = mAxisMap.find(js[i].code)->second;
                    #else
                    ...
                    //check for rescaling:
                    #if defined(__GNUC__) && defined(__INTEL_COMPILER)                    
                    if( mRanges.find(axis)->second.min == JoyStick::MIN_AXIS && mRanges.find(axis)->second.max != JoyStick::MAX_AXIS )
                    #else
                    ...
                    {   //Rescale
                        #if defined(__GNUC__) && defined(__INTEL_COMPILER)
                        float proportion = (float)(js[i].value-mRanges.find(axis)->second.max)/(float)(mRanges.find(axis)->second.min-mRanges.find(axis)->second.max);
                        #else
